...
import { SessionCallbackType } from "@/features/auth/types/callbacks-types";
...
export const sessionCallback = async ({
  ...
  if (token.sub && session.user) {
    ...
    session.user.isTwoFactorEnabled = !!token.isTwoFactorEnabled; // Associa a flag de autenticação de dois fatores ao token JWT
    session.user.tenantId = token.tenantId as string; // Associa o ID do tenant ao token JWT
  }
