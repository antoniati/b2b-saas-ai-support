...
import { UserService, TokenService } from '@/features/users';
 ...
 * @example
 * // O `signInCallback` verifica se o login é permitido para o usuário com as credenciais fornecidas:
 * const isLoginAllowed = await signInCallback({ user, account });
 *
 * // Se o usuário não tiver o e-mail verificado ou tiver 2FA habilitado sem confirmação, o login será bloqueado.
 */
export const signInCallback = async ({ user, account }: any): Promise<boolean> => {
  ...
  if (!userFound?.isEmailVerified) {
    console.warn(`Login bloqueado: E-mail não verificado (${userFound?.email})`);
    return false;
  }

  // Restringe acesso ao ADMIN apenas se for o email autorizado
  if (userFound.role === Role.ADMIN && userFound.email !== process.env.AUTH_ADMIN_EMAIL) {
    console.warn(`Tentativa de login como ADMIN não autorizado: ${userFound?.email}`);
    return false;
  }

  // Autenticação de dois fatores
  if (userFound?.isTwoFactorEnabled) {
    const { id } = await TokenService.requireByEmail(userFound?.email, TokenType.TWO_FACTOR);

    if (!id) {
      console.warn(`Login bloqueado: 2FA habilitado mas não confirmado (${userFound?.email})`);
      return false;
    }
