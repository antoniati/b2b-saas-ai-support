...
import { TokenRepository } from '@/features/users';
...
export const TokenService = {
  ...
  async generate(email: string, type: TokenType): Promise<{ token: string }> {
    ...
    if (!record) throw new DomainError(ERROR_MESSAGES.TOKEN_GENERATION_FAILED, 500);
  ...
  async delete(id: string, type: TokenType): Promise<void> {
    ...
    if (!deleted) throw new DomainError(ERROR_MESSAGES.TOKEN_DELETION_FAILED, 500);
  },
  ...
  async validateTwoFactor(email: string, code: string): Promise<void> {
    const record = await TokenService.requireByEmail(email, TokenType.TWO_FACTOR);
